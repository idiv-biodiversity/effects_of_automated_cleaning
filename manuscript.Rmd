...
- Alice Calvente^4,5^,
- Ana Paula Alves da Silva^4,5^,
- Andressa Cabral,
- Arthur de Souza Soares^4,5^,
- Eduardo Calisto Tomaz^4^,
- Fernanda Gondim Lambert Moreira^4,5^,
- Filipe Serrano,
- Jéssica Fernanda Ramos Coelho,
- Mabel Rocio Baez-Lizarazo,
- Mariana Ramos Fantinati^4,5^,
- Matheus Colli-Silva,
...
3. Departamento de Genética, Ecologia e Evolução, Instituto de Ciências Biológicas, Universidade Federal de Minas Gerais, Av. Antônio Carlos 6627 - Pampulha, 31270-901 - Belo Horizonte - MG - Brasil
4. Laboratório de Botânica Sistemática, Departamento de Botânica e Zoologia, Centro de Biociências, Universidade Federal do Rio Grande do Norte.
5. Programa de pós graduação em Sistemática e Evolução, Universidade Federal do Rio Grande do Norte, Natal, Brasil
6. Gothenburg Global Biodiversity Centre, University of Gothenburg, Box 461, 405 30 Gothenburg, Sweden
7. Department for Biological and Environmental Sciences, University of Gothenburg, Box 461, 405 30 Gothenburg, Sweden
8. Royal Botanic Gardens Kew, TW9 3AE, Richmond, Surrey, United Kingdom
...
We did all analysis in the R computing environment [@rcoreteam2019], using standard libraries for data handling and visualization [@Wickham2018; @Garnier2018; @Ooms2014; @Ooms2019; @Hijmans2019]. All analysis scripts are available from https://github.com/idiv-biodiversity/effects_of_automated_cleaning, or from the Appendix 1 in the electronic supplement of this study.
...
## Downstream analyses
We first generated species richness maps using 100x100 km grid cells for the raw and filtered dataset respectively, using the `speciesgeocodeR v2.0-10` package [@Topel2016] and then performed a preliminary conservation assessment for all study groups based on both datasets using the `ConR v1.2.4` package [@Dauby2017]. `ConR` estimates the EOO, AOO, and the number of locations, and then suggests a preliminary conservation status based on Criterion B of the global IUCN Red List. While these assessments are preliminary [see @IUCN2017], they can be a proxy used by the IUCN to speed up full assessments. We then benchmarked the preliminary conservation assessments with the global IUCN Red List assessments, which we obtained from www.iucn.org via the `rredlist v.0.5.0` package [@Chamberlain2018].
